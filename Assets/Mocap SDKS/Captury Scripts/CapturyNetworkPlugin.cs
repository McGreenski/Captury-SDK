...
namespace Captury
{
	...
	public class CapturySkeleton
	{
		...
		public static CapturyRetargetingInterface retargeter;
		...
		public GameObject mesh // reference to game object that is animated
		{
			...
			set
			{
				...
				retargetTarget = networkPlugin.ConvertGameObjectToCapturyActor(reference);
				if (retargeter != null)
					retargeter.generateMapping(rawData, retargetTarget);
			}
		}

		public System.IntPtr setPose(System.IntPtr poseData)
		{
			if (retargeter != null)
				return retargeter.retarget(rawData, retargetTarget, poseData);
			else
				return poseData;
		}
	}
	...
	// forward declaration
	public interface CapturyRetargetingInterface {
		int generateMapping(System.IntPtr srcSkel, System.IntPtr tgtSkel);
		System.IntPtr retarget(System.IntPtr srcSkel, System.IntPtr tgtSkel, System.IntPtr inputPose);
	}
    ...
    public class CapturyNetworkPlugin : MonoBehaviour
    {
		...
		// if set this function is called every time a skeleton needs to be retargeted - every frame
        ...
        void Awake()
        {
			...
			CapturySkeleton.retargeter = GetComponent<CapturyRetargetingInterface>();
        ...
        void Update()
        {
            ...
            foreach (KeyValuePair<int, CapturySkeleton> kvp in skeletons)
            {
                ...
                else
                {
                    ...
                    // convert the pose
                    CapturyPose pose;
					if (CapturySkeleton.retargeter != null && !skeletons[actorID].upToDate)
                    {
...
//                            skeletons[actorID].originalMesh = skel.originalMesh;
					...
					IntPtr retargetedPose;
					retargetedPose = skeletons[actorID].setPose(poseData);
...
//							Debug.Log ("skipping joint " + jointID + " " + skeletons[actorID].joints[jointID].name);
							continue;
						}

						Quaternion orig = skeletons[actorID].joints[jointID].originalRotation;

//						Debug.Log ("at joint " + jointID + " " + skeletons[actorID].joints[jointID].name + " " + ConvertRotation(rot));
...
/*                foreach (KeyValuePair<int, IntPtr> kvp in actorPointers)
        ...
        //===============================================
		static void ConvertActor(CapturyActor actor, IntPtr actorData, ref CapturySkeleton skel)
        {
            if (skel == null)
            {
		...
		//=============================================================================
		// Helper function to convert a game object to a CapturyActor
		//=============================================================================
		public System.IntPtr ConvertGameObjectToCapturyActor(GameObject g)
		{
			CapturyActor actor = new CapturyActor();
			...
			for (int i = 0; i < actor.numJoints; ++i)
			{
				...
				if (parent != -1)
					pos = ConvertPositionToLive(trafos[i + 1].position - trafos[i+1].parent.position);
				else
					pos = ConvertPositionToLive(trafos[i + 1].position);
				joints[i].ox = pos.x;
				joints[i].oy = pos.y;
				joints[i].oz = pos.z;
				...
				globalPositionsUnity[i] = ConvertPosition(globalPositions[i]) - trafoPos[i];
